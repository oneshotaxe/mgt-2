export default class Cursor {
    ...
    setColumnWidth(width, columnNumbers = undefined) {
        if (!columnNumbers) {
            columnNumbers = new Array(width.length)
                .fill(0)
    ...
    setRowHeight(height, rowNumbers = undefined) {
        if (!rowNumbers) {
            rowNumbers = new Array(height.length)
                .fill(0)
                .map((value, index) => index + 1);
        }
    ...
    setBordersOnArea(style, r1, c1, r2, c2) {
        this.getArea(r1, c1, r2, c2).forEach((cell) => cell.border = {
            left: { style: style },
            right: { style: style },
            top: { style: style },
            bottom: { style: style }
    ...
    setBordersAroundArea(style, r1, c1, r2, c2) {
        ...
        this.getArea(r1, c2, r2, c2).forEach((cell) => {
            cell.border ? null : cell.border = {};
            cell.border.right ? cell.border.right.style = style : cell.border.right = { style: style };
        });
        ...
        this.getArea(r2, c1, r2, c2).forEach((cell) => {
            cell.border ? null : cell.border = {};
            cell.border.bottom ? cell.border.bottom.style = style : cell.border.bottom = { style: style };
        });
    }
